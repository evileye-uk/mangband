...
extern const char * cfg_console_password;
extern const char * cfg_dungeon_master;
...
extern char *f_name;
extern char *f_text;
extern char f_char_s[MAX_F_IDX];
extern byte f_attr_s[MAX_F_IDX];
...
extern char *r_name;
extern char *r_text;
extern char r_char_s[MAX_R_IDX];
extern byte r_attr_s[MAX_R_IDX];
...
/* birth.c */
extern bool player_birth(int Ind, cptr name, cptr pass, int conn, int race, int player_class, int sex, int stat_order[]);
...
extern void map_info(int Ind, int y, int x, byte *ap, char *cp, bool server);
...
extern void display_map(int Ind);
...
extern int race_index(const char * name);
...
extern bool Destroy_connection(int Ind, const char *reason);
...
extern int Send_player_store_info(int ind, int num, char *owner, int items);
